...
## Use harvest related activity to define the species groups that are included in the timber asset account
...
```{r, echo=F}
# show the distrubution of trtcd1, trtcd2, and trtcd3
vol %>%
  group_by(trtcd1) %>%
...
```{r, echo=F}
# drop observations where trtcd1, trtcd2, or trtcd3 are all 'No observable treatment'
acct.species <- vol %>%
  filter(trtcd1 != 'No observable treatment' & 
           trtcd2 != 'No observable treatment' & 
           trtcd3 != 'No observable treatment')

# unique species groups in the timber asset account
acct.species <- acct.species %>%
  group_by(spgrp) %>%
  summarise(volume = sum(totvol, na.rm = T) / 1000000,
            .groups = 'drop') %>%
  arrange(desc(volume))

```

### What species groups do we find in the harvest group?

Its reasonable to assume that species groups that have evidence of timber management should be included in the timber asset account.


```{r}

library(knitr)
kable(acct.species, caption = "Volume for the South, in millions of cubic meters",
      digits = 0)

```
## Stumpage prices from timbermart south

```{r}
# load timbermart south prices
tms <- read.csv("data/prices_south.csv")

# pivot longer on columns 5:18
tms <- tms %>%
  pivot_longer(cols = c(4:69), values_to = "price", names_to = 'type_state_region') %>%
  mutate(product = substr(type_state_region, 1, 3),
         state = substr(type_state_region, 4, 5),
         region = substr(type_state_region, 6, 6)) %>%
  select(-type_state_region) %>%
  filter(year %in% c(2015:2020)) %>%
  group_by(product, state, type) %>%
  summarize(price = mean(price, na.rm = T), .groups = "drop")

# Add prices for pre-merchantable timber

tms$price[tms$product == 'pre'] <- tms$price[tms$product == 'plp'] / 1.05^8
```
