...
setwd("C:/Users/cmihiar/OneDrive - USDA/NCA Timber Account/Pilot South")

prices <- read.csv('Georgia stumpage prices.csv')
quantity <- read.csv('Georgia FIA example.csv')
...
p <- prices %>%
  pivot_longer(cols = c(5:10), values_to = "price") %>%
  mutate(product = substr(name, 1, 3),
...
df <- right_join(q, filter(p, product != "cns"), by = c("year", "type", "product")) %>%
...
pt1 <- PivotTable$new()
pt1$addData(filter(df, type == 'pine'))
pt1$addColumnDataGroups("state",
                       addTotal = F)
...
pt1$defineCalculation(calculationName = "tons",
                     ...
                     format="%.0f")
pt1$defineCalculation(calculationName = "price_tons",
...
pt2 <- PivotTable$new()
pt2$addData(filter(df, type == 'pine'))
...
filterOverrides <- PivotFilterOverrides$new(pt3,
                                            overrideFunction = 
                                              getLastYearFilter)
pt3$defineCalculation(calculationName = "price_tons",
                     caption = "Price Per Ton",
                     summariseExpression = "mean(price_tons, na.rm=T)",
                     format="$%.2f")
