...
def load_gl_biomass():
    ...
    # Process FIPS codes using utility function
    try:
        biomass_north = format_fips(biomass_north, state_col='STATECD', county_col='COUNTYCD')
    except ValueError as e:
        print(f"Error formatting FIPS in Great Lakes biomass data: {e}")
        biomass_north['fips'] = None
        
    # Ensure statecd is string
    if 'statecd' in biomass_north.columns:
        biomass_north['statecd'] = biomass_north['statecd'].astype(str).str.zfill(2)
        
    # Ensure unitcd is string
    if 'unitcd' in biomass_north.columns:
        biomass_north = format_unit_code(biomass_north, unit_col='UNITCD') # Assuming format_unit_code handles case or standardized name
        # Or standardize first: biomass_north = format_unit_code(standardize_column_names(biomass_north), unit_col='unitcd')

    # Standardize column names AFTER potential FIPS creation if needed
    biomass_north = standardize_column_names(biomass_north)

    # Select needed columns (adjust based on actual standardized names)
    final_cols = {}
    mapping = {
        'year': 'year', 
        'statenm': 'statenm',
    ...
    for target, source in mapping.items():
        if source in biomass_north.columns:
            final_cols[target] = biomass_north[source]
        else:
            print(f"Warning: Column '{source}' not found in GL biomass data for final selection.")
            final_cols[target] = None 
